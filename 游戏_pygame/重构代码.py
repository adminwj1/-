...
class HeroPlan(BasePlane):
    ...
    def move(self):
        ...
        if self.key_left_status:
            self.x -= 3
            print("x是：, y是：", self.x, self.y)   # 测试输出飞机的x和y轴的值
        if self.key_right_status:
            self.x += 3
            print("x是：, y是：", self.x, self.y)
    ...
    def crash(self):
        pygame.sprite.spritecollide(Bullet, EnemyPlane, True, True)

class Bullet(BaseBullet, pygame.sprite.Sprite):
    """玩家子弹类"""
    def __init__(self, screen_temp, x, y):  # 显示主界面
        ...
        self.rect = self.image.get_rect()  # 获取加载图片的大小
        print(self.rect)
    def move(self):
        self.y -= 10
...
class EnemyPlane(BasePlane):
    ...
    def fire(self):
        bullte = random.randint(1, 200)
        if bullte == 100 or bullte == 50 or bullte == 200:
